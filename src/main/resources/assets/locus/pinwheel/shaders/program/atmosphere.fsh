...
vec3 calculateLight(vec3 rayOrigin, vec3 rayDir, float rayLength, vec3 originalCol) {
    ...
    for (int i = 0; i < numInScatteringPoints; i ++) {
        vec2 intersection = boxIntersection(inScatterPoint, dirToSun, vec3(planetRadius,planetRadius,planetRadius));
...
void main() {
    ...
    if (dstThroughAtmosphere > 0) {
        ...
        vec3 light = calculateLight(pointInAtmosphere, rayDir, dstThroughAtmosphere - epsilon * 2, vec3(originalCol.x, originalCol.y, originalCol.z));
